...
// destructor
{
	if (x) x = NULL; // nullptr? this is getting messed up in moodle
	if (y) y = NULL;
}

istream& operator>>(istream& is, Point& pt) // added the &!!
// method to read in a point in (x,y) form
{
	char lp, comma, rp;
	int xvalue, yvalue;
	is >> lp >> xvalue >> comma >> yvalue >> rp;
	pt.setxValue(xvalue);
	pt.setyValue(yvalue);
	return is;
}
